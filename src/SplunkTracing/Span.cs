...
namespace SplunkTracing
{
    ...
    public sealed class Span : ISpan
    {
            ...
            IReadOnlyCollection<Reference> references)
        {
            ...
            {
                _tracer.BuildSpan(SplunkTracingConstants.MetaEvent.TracerCreateOperation)
                    .IgnoreActiveSpan()
                    .WithTag(SplunkTracingConstants.MetaEvent.MetaEventKey, true)
                    .WithTag(SplunkTracingConstants.MetaEvent.SpanIdKey, _context.SpanId)
                    .WithTag(SplunkTracingConstants.MetaEvent.TraceIdKey, _context.TraceId)
                    .Start()
                    .Finish();
            }
        ...
        private void OnFinished()
        {
            ...
            if(_tracer._options.EnableMetaEventLogging && Utilities.IsNotMetaSpan(this))
            {
                _tracer.BuildSpan(SplunkTracingConstants.MetaEvent.SpanFinishOperation)
                    .IgnoreActiveSpan()
                    .WithTag(SplunkTracingConstants.MetaEvent.MetaEventKey, true)
                    .WithTag(SplunkTracingConstants.MetaEvent.SpanIdKey, this.TypedContext().SpanId)
                    .WithTag(SplunkTracingConstants.MetaEvent.TraceIdKey, this.TypedContext().TraceId)
                    .Start()
                    .Finish();
            } 
        }
